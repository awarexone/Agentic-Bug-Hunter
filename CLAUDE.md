...
| `/bench` | `/bench run [--min-precision 1.0 --max-fp-rate 0.0] \| list` — BugBench: measure detector precision/recall/false-positive rate vs a ground-truth corpus (known-vuln + known-safe); `--min-precision`/`--max-fp-rate` gate CI against quality regressions |
...
- `tools/bench.py` — BugBench detection-quality harness. Runs the toolkit's detectors against a ground-truth corpus (`bughunter/bench/cases/*.json`, known-vuln + known-safe, recorded inputs = deterministic) and reports precision / recall / false-positive rate per detector. Detectors register a one-function adapter via `@detector`; cases are contributable JSON. `run` / `list`; `--min-precision`/`--max-fp-rate` exit non-zero to gate CI against quality regressions. Pure-stdlib. See `/bench`.
